...
class Cam( ):
    ...
    def intrinsic_matrix(self, yl = 0.0, k1 = 0.0, k2 = 0.0, k3 = 0.0):
        """
        Function that returns the intrinsic matrix of the camera
        
        :param yl: ratio between y-coordinate and z-coordinate of the point in the camera coordinate system
        :type yl: float
        ...
        if theta_d != 0.0 and yl != 0.0:
            q = theta_d/yl # y-axis correction factor (because of camera distortion)
        else:
            q = 1.0
        ...
        my = self.y_resolution/(camera_height*aspect_ratio_y)*scale*q
...
class Image_object:
    ...
    def to_image_coord(self, point):
        ...
        K = self.camera.intrinsic_matrix(yl = point_camera_coord[1]/point_camera_coord[2], k1 = -0.12, k2 = 0.03, k3 = 0.01)

        point_image_coord = K @ point_camera_coord
    ...
    def get_bounding_box(self):
        ...
        if self.box.max_x < 2/5*self.box.width or self.box.min_x > 1 - 2/5*self.box.width or self.box.max_y < 2/5*self.box.height or self.box.min_y > 1 - 2/5*self.box.height:
            return (0.0, 0.0, 0.0, 0.0)
        else:
